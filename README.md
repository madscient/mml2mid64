...
変わりません。生成MIDIが変わりうる不具合修正が1件あります）。

オリジナル配布物のうち、ドキュメント（`org-doc/`）、プリプロセッサ（`mmlpp/`）、
GUIフロントエンド（`tk/`）、および配布物の説明（[readme.txt](readme.txt)）は
同梱しています。
...
- **同じ行で主トラックより後ろに書いた従属トラックが捨てられていた問題**を修正
  ...
  正しく動いていました）。**本フォークで唯一、生成MIDIが変わりうる修正**です。

### MMLの拡張

既存のMMLの解釈は変えないことを原則としつつ、2点だけ機能を追加しています。

- **`P` / `X` コマンドが 0〜4 の引数を取れるようになりました。** ダンパー・ペダル
  （CC64）に加え、ポルタメント（65）、ソステヌート（66）、ソフト・ペダル（67）、
  レガート（68）のオン・オフができます。引数を省略すると従来どおりダンパー・ペダル
  なので、既存のMMLへの影響はありません。
- **トラック名に2文字目の数字を書けるようになりました。** `A0`〜`Z9` が使え、主
  トラックが26本から286本に増えます。1文字のトラック名や `AE` のような並列表記の
  意味は変わりません。あわせてトラック数の上限を64本から2860本に引き上げ、SMF
  ヘッダのトラック数が1バイトでしか書かれていなかった問題も直しています。

詳細は [doc/CHANGES.md](doc/CHANGES.md#mmlの拡張) を参照してください。
...
非公開化します。オリジナルの配布物の説明は [readme.txt](readme.txt) にあります。
