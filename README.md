...
ようにモダナイズしたものです。**生成されるMIDIの内容はオリジナルと変わりません**
（[1点の拡張](#mmlの拡張)を除く）。
...
回帰テストは `sample/*.mml` をすべてコンパイルし、`test/baseline.sha256` に記録
されたSHA-256と照合します。生成MIDIが変化する変更は即座に検出されます。

**サンプル曲は同梱していないため、既定ではテストは無効です**（ビルド自体は正常に
行えます。CMake がその旨のメッセージを表示します）。有効にするには、
...
| `test/` | 回帰テスト用のベースラインとスクリプト（サンプル曲は非同梱） |
