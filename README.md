...
**不具合修正のテスト。** `test/regress/*.mml` をコンパイルし、`test/regress.sha256`
のSHA-256と照合します。オリジナル版のTODOリスト（[org-doc/todo.txt](org-doc/todo.txt)）
にあった不具合の修正と、その修正で**変わってはいけない**ケースの両方を固定します。
これも本フォークで書いたMMLなので常に実行されます。
...
（オリジナル版が生成したもの）と照合して検証済みです。**32件中28件がバイト単位で
完全一致**しました。一致しない4件の原因と根拠は
[test/README.md](test/README.md) に記載しています（うち3件は `.mid` 生成時の
テンポ値の丸め方の違い、1件は乱数ベロシティによる非決定性で、いずれも本リポジトリ
での退行ではありません）。
