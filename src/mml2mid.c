...
MML_NORETURN void remove_file_and_owari(void);
MML_NORETURN static void remove_file_and_owari0(void);
MML_NORETURN static void remove_file_and_owari1(void);
MML_NORETURN static void remove_file_and_owari2(void);
...
MML_NORETURN static void prepro_illstring(prepro_linebuf *);
MML_NORETURN static void prepro_illcont(prepro_linebuf *);
MML_NORETURN static void prepro_nomem(void);
...
static char *strnDup(const char *s, size_t n)
{
	...
	if(l < n) n = l;
	if(NULL != (p = malloc(n + 1))) memcpy(p, s, n), p[n] = '\0';
	return p;
}

static char err_mem[] = "ERROR! Insufficient memory\n";

static void add_env_options(const char *envname, int *argcp, char ***argvp)
{
	char	*env, *p;
...
# define VER_TARGET " (Linux version)"
#else
   ...
   始まらない2文字以下の場合にそこ以降を無条件にオプション引数扱いする */
	...
	if((fp0 = fopen2(srcfile, "rb")) == NULL) do {
		if(strchr(base, '.') == NULL){ /* 拡張子なしなら「.mml」を付け再try */
			char *file1;
			size_t sz = strlen(srcfile) + sizeof(MML_EXT);

			if((file1 = malloc(sz)) == NULL){
				fputs(err_mem, STDERR);
				owari();
			}
			snprintf(file1, sz, "%s%s", srcfile, MML_EXT);
			fp0 = fopen2(file1, "rb");
			free(file1);
	...
	if(outfile == NULL){ /* まだ出力ファイル名が決まっていなければ */
		if(!strcmp(srcfile, stdio_name)){
			...
			size_t sz = strlen(srcfile) + sizeof(MID_EXT);
			...
			if(outfile == NULL){
	 		...
	 		memcpy(outfile, srcfile, strlen(srcfile) + 1);
			if(strchr(base, '.') == NULL){
...
MML_NORETURN static void sw(void)   /* Usage */
{
	fprintf(STDERR,
...
static void fputs2(const char *s, fileptr fp)
 /* 本来はfile.cで実現するべき */
{
	for(; *s; s++) putc2(*s, fp);
}
...
static void smftrkend(fileptr fp, Fpos_t *trsize)
{
	...
	putc2(0, fp);
	fgetpos2(fp, &trkend);
	j = trkend - *trsize - 4; /* たぶん -4 でいいと思う */
	fsetpos2(fp, trsize);
...
static void puttitle(const char *title, const char *copyright)
{
	 /* The length is written as a single byte, so gettitle() rejects
	    anything longer than 255. */
	putc2(0xff, fp3);
	putc2(3, fp3);
	if(title == NULL) title = "";
	putc2((int)strlen(title), fp3);
	fputs2(title, fp3);
	...
	putc2((int)strlen(copyright), fp3);
...
static void analyze(void)
{
	...
	if((fp1 = fmalloc()) == NULL){
		msg_printf("ERROR! Cannot allocate memory (for fp1)\n");
		text_cat(Msg);
		fclose2(fp0);
		remove_file_and_owari0();
	}
...
static int getLine(prepro_linebuf *lbuf, int i)
{
	...
	for(;;){
		if(i >= lbuf->amount){
			...
			nb = realloc(lbuf->buf, (size_t)(i + 80));
			if(nb == NULL) return -1;
			lbuf->amount = i + 80;
			lbuf->buf = nb;
		}
...
MML_NORETURN static void prepro_illstring(prepro_linebuf *lbuf)
{
	prepro_error("ERROR! string not closed or illegal '\\' escape,", lbuf);
}
MML_NORETURN static void prepro_illcont(prepro_linebuf *lbuf)
{
...
static void
	out_ppinfo(fileptr fpo, int lineno, const char *curfile,
		   const char *virt_curfile)
{
	char numbuf[32];
...
static void gettitle(const char *kind, char *p, prepro_linebuf *lbuf)
{
	char *q;
	...
	if(*next_nonsp(q+1)) prepro_illdirective(lbuf);
	 ...
	 /* The SMF meta-event stores the length in one byte, so 255 is the
	    real limit; the old test allowed 256 and then wrote a length of 0. */
	if(strlen(p) > 255){
		msg_printf("ERROR! %s too long", kind);
		prepro_error(Msg, lbuf);
	}
...
static void format1to0(void)
{
	...
	if((fp4 = fmalloc()) == NULL ){
		msg_printf("ERROR! Cannot allocate memory (for fp4)\n");
		text_cat(Msg);
		remove_file_and_owari2();
	}
	...
	for(;;){
		...
		if(code == EOF){
			msg_printf("error occured while converting format 1 -> 0.\n");
