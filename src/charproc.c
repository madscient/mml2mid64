...
static void reallocmacro(macrostruct *m, int i)
{
	unsigned char *p = (unsigned char *)realloc(m->mac, (size_t)i);

	if(p == NULL) mml_err(61);
	m->mac = p;
	m->alcsiz = i;
}
...
static void macrocat(macrostruct *to, macrostruct *from){
	 /* マクロtoの後ろにfromを追加 但しtoの末尾にNULLの自動追加はしない */
	int needed = to->uselen + from->uselen - 1;
	macrosizchk(to, needed);
	memcpy(&to->mac[to->uselen], from->mac, (size_t)(from->uselen - 1));
...
void free_all_macros(void)
{
	int i;
	 ...
	 /* An error can be raised before init_all_macros() runs (out of memory
	    during preprocessing, say), and the error path calls this. */
	if(mcrstr == NULL) return;

	for(i = 0; i < 10 * 26; i++) free(macros(i));
	free(mcrstr);
	mcrstr = NULL;
}
...
static void check_master_step_amount(void)
{
	int i;
	int prev_amount;
